...
import { BellRing, Camera, CameraOff, Loader2, Mic, MicOff, Phone, PhoneOff, Play, RotateCcw, Video, X } from 'lucide-react';
...
export default function VideoConsultationRoom({ appointmentId, role, otherPartyName, consultationMethod = 'platform', compact = false }: Props) {
  ...
  useEffect(() => {
    const handleForegroundCall = (event: Event) => {
      const activeAppointmentId = (event as CustomEvent<{ appointmentId: string }>).detail?.appointmentId;
      setIsForeground(activeAppointmentId === appointmentId);
    };
    window.addEventListener('medibook:active-video-call', handleForegroundCall);
    return () => window.removeEventListener('medibook:active-video-call', handleForegroundCall);
  }, [appointmentId]);

  const ensureMedia = useCallback(async () => {
    const existing = localStreamRef.current;
    const hasLiveVideo = existing?.getVideoTracks().some((track) => track.readyState === 'live');
    const hasLiveAudio = existing?.getAudioTracks().some((track) => track.readyState === 'live');
  ...
  const ensurePeer = useCallback(async () => {
    ...
    peer.onicecandidate = (event) => {
      if (event.candidate) socketRef.current?.emit('webrtc-signal', { appointmentId, signal: { candidate: event.candidate } });
    };
    peer.ontrack = (event) => {
  ...
  }, [appointmentId, ensurePeer, resetPeer]);
  ...
  const uploadRecording = useCallback(
    async (blob: Blob) => {
      ...
      try {
        ...
        form.append('file', new File([blob], `consultation-${appointmentId}.webm`, { type: blob.type || 'video/webm' }));
        const response = await fetch(`${serverUrl}/doctor/video-recording/${appointmentId}`, {
          method: 'POST',
          headers: {
        ...
        if (!response.ok) throw new Error('Could not save consultation recording.');
  ...
  const startRecording = useCallback(() => {
    if (role !== 'doctor' || recorderRef.current || !localVideoRef.current || !remoteVideoRef.current) return;
    ...
    const draw = () => {
      ...
      if (remote?.readyState && remote.videoWidth) context.drawImage(remote, 0, 0, canvas.width, canvas.height);
      if (local?.readyState && local.videoWidth) {
        context.drawImage(local, canvas.width - 330, canvas.height - 205, 300, 175);
      }
    ...
    const recorder = new MediaRecorder(recordedStream, { mimeType, videoBitsPerSecond: 700_000 });
  ...
  useEffect(() => {
    ...
    getToken().then((token) => {
      ...
      liveSocket.on('call-state', (state) => {
        ...
        setRemainingSeconds(Math.max(0, Math.ceil(Number(state.remainingMs || 0) / 1000)));
              ...
              if (role === 'doctor') {
                window.setTimeout(() => {
                  sendOffer()
                    ...
                    .catch((reconnectError) => setError(reconnectError.message || 'Could not reconnect the video call.'));
                }, 300);
              }
            })
            .catch((mediaError) => setError(mediaError.message || 'Camera and microphone permission is required.'));
        }
      ...
      liveSocket.on('incoming-call', (payload) => {
        ...
        setStatus('ringing');
        setEndsAt(null);
        setRemainingSeconds(Math.max(0, Math.ceil(Number(payload.durationMs || 0) / 1000)));
        if (role === 'patient') {
          setIsOpen(true);
          if (typeof Notification !== 'undefined' && Notification.permission === 'granted') {
            new Notification('Incoming video consultation', {
      ...
      liveSocket.on('call-accepted', async (payload) => {
        ...
        setRemainingSeconds(Math.max(0, Math.ceil(Number(payload.remainingMs || 0) / 1000)));
        setIsOpen(true);
        if (role === 'doctor') {
          await sendOffer();
      ...
      liveSocket.on('webrtc-signal', async ({ signal }) => {
        try {
          let peer = await ensurePeer();
          if (signal.description) {
            ...
            if (signal.description.type === 'offer') {
              ...
              liveSocket?.emit('webrtc-signal', { appointmentId, signal: { description: peer.localDescription } });
            }
          } else if (signal.candidate) {
            await peer.addIceCandidate(signal.candidate).catch(() => undefined);
          }
        } catch (signalError: any) {
          setError(signalError.message || 'Could not reconnect the video call.');
        }
      });
      ...
      liveSocket.on('participant-joined', ({ role: joinedRole }) => {
        if (role === 'doctor' && joinedRole === 'patient' && (statusRef.current === 'active' || statusRef.current === 'paused')) {
          window.setTimeout(() => {
            sendOffer()
              .then(() => window.setTimeout(startRecording, 1200))
              .catch((reconnectError) => setError(reconnectError.message || 'Could not reconnect the patient video.'));
          }, 300);
        }
      });
      liveSocket.on('call-paused', ({ pausedBy: who, remainingMs }) => {
        ...
        setPausedBy(who);
        setEndsAt(null);
        setRemainingSeconds(Math.max(0, Math.ceil(Number(remainingMs || 0) / 1000)));
      });
      liveSocket.on('resume-requested', () => setResumeRequested(true));
      liveSocket.on('call-resumed', ({ endsAt: resumedEndsAt, remainingMs }) => {
        statusRef.current = 'active';
        setStatus('active');
      ...
      liveSocket.on('call-missed', ({ message }) => {
        ...
        setRemaining('');
        setError(message || 'The call invitation expired. Wait for the doctor to call again.');
        endLocalMedia();
      });
      liveSocket.on('call-ended', ({ reason }) => {
        claimForeground();
        ...
        setRemainingSeconds(0);
        setRemaining('00:00');
        setEndedReason(reason === 'time-completed' ? 'Consultation time completed.' : 'The doctor ended the consultation.');
        setIsOpen(true);
        if (role === 'patient' && typeof Notification !== 'undefined' && Notification.permission === 'granted') {
          new Notification('Video consultation ended', {
            body: reason === 'time-completed' ? 'Your consultation time has completed.' : 'The doctor ended the consultation.',
  ...
  }, [appointmentId, claimForeground, endLocalMedia, ensurePeer, resetPeer, role, sendOffer, serverUrl, startRecording]);
  ...
  if (consultationMethod === 'whatsapp') {
    return role === 'doctor' ? <span className="text-xs font-bold text-emerald-700">Patient selected WhatsApp</span> : null;
  }

  return (
    <>
      {role === 'doctor' && (status === 'scheduled' || status === 'ringing') && (
        <button
          onClick={doctorStart}
        ...
        >
          {isPreparing ? <Loader2 size={16} className="animate-spin" /> : <Video size={16} />}{' '}
          {status === 'ringing' ? 'Call Patient Again' : 'Start Platform Video'}
        </button>
      )}
      {role === 'patient' && notificationPermission !== 'granted' && status !== 'completed' && (
        <button
          type="button"
          onClick={async () => {
      ...
      {role === 'patient' && status === 'active' && (!isOpen || !isForeground) && (
        <button
          onClick={() => {
            claimForeground();
            setIsOpen(true);
      ...
      {isOpen &&
        ...
        createPortal(
          <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-950/90 p-3 backdrop-blur">
            <div className="relative flex h-full max-h-[850px] w-full max-w-6xl flex-col overflow-hidden rounded-3xl bg-slate-900 shadow-2xl">
              <div className="flex items-center justify-between border-b border-white/10 px-5 py-3 text-white">
                <div>
                  ...
                  <p className="text-xs text-slate-400">
                    {status === 'ringing' ? 'Calling…' : status === 'paused' ? 'Video paused' : 'Secure platform consultation'}
                  </p>
                </div>
                <div className="flex items-center gap-3">
                  <span className="font-mono text-sm text-teal-300">{remaining}</span>
                  {status === 'completed' && (
              ...
              <div className="relative min-h-0 flex-1 bg-black">
                ...
                {status === 'completed' && (
                  <div className="absolute inset-0 z-20 flex flex-col items-center justify-center bg-slate-950/95 px-6 text-center text-white">
                    ...
                    <p className="mt-2 text-sm text-slate-300">{endedReason || 'The video consultation has ended.'}</p>
                    <button onClick={() => setIsOpen(false)} className="mt-6 rounded-xl bg-white px-6 py-2.5 font-bold text-slate-900">
                      Close
                    </button>
                  </div>
                )}
                {status === 'paused' && (
                  <div className="absolute inset-0 flex flex-col items-center justify-center bg-black/70 text-white">
                    <CameraOff size={50} />
                    <p className="mt-3 text-xl font-bold">{pausedBy === role ? 'Your video is paused' : `${otherPartyName}'s video is paused`}</p>
                    {pausedBy !== role && (
                      <button
                        onClick={() => socket?.emit('resume-request', { appointmentId })}
                ...
                {incoming && role === 'patient' && (
                  <div className="absolute inset-0 flex flex-col items-center justify-center bg-slate-950/90 text-white">
                    ...
                    <h2 className="mt-5 text-2xl font-bold">Incoming call from {otherPartyName}</h2>
                    <p className="mt-2 text-sm text-slate-400">Accepting will turn on your camera and microphone.</p>
                    <button
                      onClick={patientAccept}
                      disabled={isPreparing}
                      className="mt-6 inline-flex items-center gap-2 rounded-full bg-emerald-500 px-7 py-3 font-bold"
                    >
                      {isPreparing ? <Loader2 className="animate-spin" /> : <Phone />} Accept Video Call
                    </button>
                  </div>
                )}
              </div>
              <div className="flex flex-wrap items-center justify-center gap-3 border-t border-white/10 bg-slate-950 px-4 py-4">
                ...
                {status === 'paused' && pausedBy === role ? (
                  <button onClick={resume} className="rounded-full bg-emerald-500 p-3 text-white">
                    <Play />
                  </button>
                ) : status === 'active' ? (
                  <button onClick={pause} className="rounded-full bg-white/10 p-3 text-white">
                    {cameraEnabled ? <Camera /> : <CameraOff />}
                  </button>
                ) : null}
                {role === 'doctor' && (status === 'active' || status === 'paused') && (
                  <button
                    onClick={() => socket?.emit('doctor-end-call', { appointmentId })}
              ...
              {error && <div className="bg-red-600 px-4 py-2 text-center text-sm text-white">{error}</div>}
            </div>
          </div>,
          document.body,
        )}
