// app/page.tsx
import { CalendarDays, Users, Wallet, Star, Bell, Search, CheckCircle, Video, MapPin, MoreHorizontal, ChevronRight, LogOut, Heart, ArrowRight } from 'lucide-react';
  ...
  <div className="flex items-center justify-between p-5 bg-white border border-slate-100 rounded-2xl hover:shadow-elevated hover:border-[#16BCC8]/10 transition-all duration-300 group">
    ...
    <div className="flex items-center gap-6">
      <div className="text-right hidden sm:block">
        ...
        <span className={`text-[10px] uppercase font-bold tracking-wider ${apt.status === 'Upcoming' ? 'text-blue-600' : apt.status === 'Completed' ? 'text-[#20AC6B]' : 'text-red-500'}`}>
          {apt.status}
        </span>
      </div>
        ...
        {apt.status === 'Upcoming' && (
          <>
            ...
            <Link
              ...
              className="px-4 py-2 text-sm font-semibold text-white bg-gradient-to-r from-[#16BCC8] to-[#0ea5a9] rounded-xl transition-all duration-300 shadow-[0_2px_8px_rgba(22,188,200,0.3)] hover:shadow-[0_4px_16px_rgba(22,188,200,0.4)]"
            >
              Manage
            </Link>
          </>
...
export default async function DoctorDashboard() {
  ...
  const specializedAppointments: Appointment[] = patientsData.map((item: any, idx: number) => {
    ...
    const timeFormatted = item.startTime ? new Date(item.startTime).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : '09:00 AM';
      ...
      <header className="sticky top-0 z-10 bg-white/80 backdrop-blur-xl border-b border-slate-100 px-4 sm:px-8 py-4 sm:py-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-4">
          <div className="hidden md:flex items-center bg-slate-50 rounded-xl px-4 py-2.5 w-64 border border-transparent focus-within:border-[#16BCC8]/30 focus-within:ring-2 focus-within:ring-[#16BCC8]/10 transition-all duration-200">
            ...
            <input type="text" placeholder="Search appointments..." className="bg-transparent border-none outline-none text-sm ml-2 w-full text-slate-700 placeholder:text-slate-300" />
          </div>
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="space-y-4">
              {specializedAppointments.length === 0 ? (
                ...
                specializedAppointments.map((apt) => <AppointmentCard key={apt.id} apt={apt} />)
              )}
            </div>
          </div>
            ...
            <div className="bg-white rounded-2xl p-6 border border-slate-100 shadow-card">
              <div className="flex items-center justify-between mb-6">
                ...
                <Link
                  ...
                  className="text-xs bg-slate-50 hover:bg-slate-100 px-3 py-1.5 rounded-lg transition-colors duration-200 font-semibold text-slate-500 border border-slate-100"
                >
                  Edit
                </Link>
              </div>
              <div className="space-y-4">
                ...
                <div className="mt-4 pt-4 border-t border-slate-100 flex justify-between text-xs text-slate-400 font-medium">
                  <span className="flex items-center gap-2">
                    <span className="w-2.5 h-2.5 rounded-full bg-[#16BCC8]"></span> Available
                  </span>
                  <span className="flex items-center gap-2">
                  ...
                  <div key={review.id} className="pb-6 border-b border-slate-50 last:border-0 last:pb-0">
                    <div className="flex justify-between items-start mb-2">
                      <div className="flex items-center gap-2.5">
                        <div className="w-9 h-9 rounded-xl bg-indigo-50 text-indigo-600 flex items-center justify-center font-bold text-xs">{review.patientName.charAt(0)}</div>
