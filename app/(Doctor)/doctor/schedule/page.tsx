...
export default function DoctorSchedule() {
  ...
  const [allowWhatsAppVideoConsultation, setAllowWhatsAppVideoConsultation] = useState(false);
  ...
  const fetchDoctorProfile = async () => {
    try {
      ...
      if (profile) {
        ...
        setAllowWhatsAppVideoConsultation(profile.allowWhatsAppVideoConsultation === true);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await axios.post(
        `${serverUrl}/doctor/updateAvailability`,
        {
    ...
    <DashboardShell role="doctor" activeHref="/doctor/schedule" showHealthTip={false}>
      <header className="sticky top-0 z-10 bg-white/80 backdrop-blur-xl border-b border-slate-100 px-4 sm:px-6 py-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <img
            src={`https://ui-avatars.com/api/?name=${encodeURIComponent(doctorName)}&background=0D9488&color=fff`}
            alt="Profile"
      ...
      {isLoading ? (
        ...
        <div className="p-4 md:p-6 max-w-[1600px] mx-auto grid grid-cols-1 xl:grid-cols-4 gap-6">
              ...
              <div className="space-y-6">
                ...
                <div>
                  ...
                  <div className="space-y-3">
                    ...
                    <label className="flex cursor-pointer items-start gap-3 rounded-xl border border-emerald-100 bg-emerald-50/60 p-3">
                      <input
                        type="checkbox"
                        checked={allowWhatsAppVideoConsultation}
                        disabled={!isVideoEnabled}
                        onChange={(event) => setAllowWhatsAppVideoConsultation(event.target.checked)}
                        className="mt-1 h-4 w-4 rounded border-slate-300 text-emerald-600 focus:ring-emerald-500 disabled:opacity-50"
                      />
                      <span>
                        <span className="block text-sm font-semibold text-slate-700">Allow WhatsApp video calls</span>
                        <span className="mt-1 block text-xs leading-5 text-slate-500">Patients can choose WhatsApp only when this permission is checked.</span>
                      </span>
                    </label>
                    <div className="flex items-center justify-between p-3 bg-slate-50/80 rounded-xl border border-slate-100">
                      <div className="flex items-center gap-2.5 text-sm text-slate-700 font-medium">
          ...
          <div className="xl:col-span-3">
            <div
              className="bg-white border border-slate-100 rounded-2xl shadow-card overflow-hidden flex flex-col h-full min-h-[600px] animate-fade-up"
