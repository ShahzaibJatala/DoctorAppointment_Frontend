...
export default function AppointmentDetailsPage() {
  ...
  useEffect(() => {
    const handleRecordingSaved = (event: Event) => {
      ...
      if (detail?.appointmentId === params.appointmentId && detail.url) {
        setAppointment((current) => (current ? { ...current, videoRecordingUrl: detail.url } : current));
      }
    };
  ...
  if (loading) {
    return (
      <DashboardShell role="patient" activeHref="/patient/appointments">
        <div className="p-8 text-slate-500">Loading appointment details...</div>
      </DashboardShell>
    );
  }
  ...
  if (!appointment) {
    return (
      <DashboardShell role="patient" activeHref="/patient/appointments">
        <div className="p-8">
          <p className="mb-4 text-slate-600">Appointment not found.</p>
          <Link href="/patient/appointments" className="text-teal-600">
  ...
  const rows = [
    ...
    {
      label: 'Time',
      value: `${start.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}${end ? ` - ${end.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}` : ''}`,
      icon: Clock,
    },
        ...
        <div className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
            ...
            {rows.map(({ label, value, icon: Icon }) => (
              <div key={label} className="flex gap-3 rounded-2xl border border-slate-100 bg-slate-50 p-4">
                ...
                <div>
                  ...
                  <p className="mt-1 font-semibold capitalize text-slate-800">{value}</p>
                </div>
              </div>
            ))}
          </div>
          <div className="border-t border-slate-100 p-5 sm:p-8">
            ...
            <div className="grid gap-3 text-sm sm:grid-cols-2">
              <p>
                <span className="text-slate-400">Name:</span> <b>{appointment.patientName || 'Not provided'}</b>
              </p>
              <p>
                <span className="text-slate-400">Age:</span> <b>{appointment.patientAge || 'Not provided'}</b>
              </p>
              <p>
                <span className="text-slate-400">Phone:</span> <b>{appointment.patientPhone || 'Not provided'}</b>
              </p>
              <p>
                <span className="text-slate-400">Gender:</span> <b>{appointment.patientGender || 'Not provided'}</b>
              </p>
              {appointment.tokenNumber && (
                <p>
                  <span className="text-slate-400">Token:</span> <b>#{appointment.tokenNumber}</b>
                </p>
              )}
            </div>
            {appointment.bankTransferReceiptUrl && (
              <a href={appointment.bankTransferReceiptUrl} target="_blank" rel="noreferrer" className="mt-5 inline-flex text-sm font-bold text-teal-600 hover:underline">
            ...
            {appointment.videoRecordingUrl && (
              <div className="mt-6 border-t border-slate-100 pt-5">
                ...
                {showRecording && (
                  <video controls preload="metadata" className="mt-4 max-h-[480px] w-full rounded-xl bg-black">
                    <source src={appointment.videoRecordingUrl} />
                  </video>
                )}
              </div>
            )}
