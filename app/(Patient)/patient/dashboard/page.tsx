...
const QuickActionCard = ({ icon: Icon, title, desc, color, href }: { icon: any; title: string; desc: string; color: string; href: string }) => (
  <Link
    href={href}
    className="flex flex-col items-start p-5 bg-white border border-slate-100 rounded-2xl hover:shadow-elevated hover:border-[#16BCC8]/15 hover:-translate-y-0.5 transition-all duration-300 text-left w-full group"
  >
      ...
      <div>
        ...
        <div className="flex items-center gap-3 mt-1.5 text-xs font-medium text-slate-400">
          <span className="flex items-center gap-1">
            <Calendar size={11} /> {new Date(apt.date).toLocaleDateString([], { month: 'short', day: 'numeric', year: 'numeric' })}
          </span>
          <span className="flex items-center gap-1">
            <Clock size={11} /> {new Date(apt.time).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
          </span>
        </div>
      </div>
    </div>
    ...
    <div className="flex items-center justify-between md:gap-8 w-full md:w-auto">
      ...
      <div className="flex items-center gap-2 md:ml-6">
        <Link
          ...
          className="px-4 py-2 text-sm font-semibold text-[#16BCC8] bg-[#16BCC8]/8 hover:bg-[#16BCC8]/15 rounded-xl transition-all duration-200 border border-[#16BCC8]/10"
        >
          View
        </Link>
      </div>
...
export default function PatientDashboard() {
      ...
      try {
        ...
        if (aptRes.ok) {
          ...
          const mapped = aptData.map((item: any) => ({
            ...
            date: item.startTime,
            time: item.startTime,
            type: ['Video', 'Online'].includes(item.type || item.appointmentType) ? 'Video' : 'In-Clinic',
            status: item.status,
  ...
  const upcomingApts = appointments.filter((a) =>
    ['confirmed', 'pending', 'checked-in', 'in-progress', 'upcoming'].includes(String(a.status || '').toLowerCase()),
  );

  // Recent prescriptions from medical records
  const recentPrescriptions = profile?.medicalRecords ? [...profile.medicalRecords].reverse().slice(0, 3) : [];

  const stats = [
    { label: 'Upcoming Appts', value: String(upcomingApts.length).padStart(2, '0'), icon: Calendar, color: 'text-[#16BCC8]', bg: 'bg-[#16BCC8]/8' },
    {
      label: 'Completed Visits',
      value: String(profile?.medicalRecords?.length || 0).padStart(2, '0'),
      icon: ShieldCheck,
      color: 'text-blue-600',
      bg: 'bg-blue-50',
    },
    {
      ...
      value: String(recentPrescriptions.filter((r: any) => r.prescription).length || 0).padStart(2, '0'),
      icon: FileText,
      color: 'text-rose-500',
      bg: 'bg-rose-50',
    },
  ...
  return (
    <DashboardShell role="patient" activeHref="/patient/dashboard">
      {upcomingApts
        .filter((appointment) => appointment.type === 'Video' && appointment.videoConsultationMethod !== 'whatsapp')
        .map((appointment) => (
          <VideoConsultationRoom
            key={appointment.id}
            appointmentId={appointment.id}
            role="patient"
            otherPartyName={appointment.doctorName}
            consultationMethod={appointment.videoConsultationMethod}
            compact
          />
        ))}
        ...
        <div className="p-6 max-w-7xl mx-auto space-y-8 animate-fade-up">
              ...
              <div className="relative flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                <div>
                  ...
                  <p className="text-white/70 text-sm mt-1">
                    You have a visit scheduled with {upcomingApts[0].doctorName} on{' '}
                    {new Date(upcomingApts[0].date).toLocaleDateString([], { month: 'short', day: 'numeric' })} at{' '}
                    {new Date(upcomingApts[0].time).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}.
                  </p>
                </div>
                <div className="flex gap-3">
                  <Link
                    href="/patient/appointments"
                    className="px-5 py-2.5 bg-white text-[#16BCC8] rounded-xl text-sm font-bold hover:bg-white/90 transition-all duration-200 shadow-sm"
                  >
                    View Details
                  </Link>
                </div>
          ...
          <section className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-5">
            {stats.map((stat, index) => (
              <div
                key={index}
                className="bg-white p-5 md:p-6 rounded-2xl border border-slate-100 shadow-card flex flex-col items-center text-center hover:shadow-elevated hover:-translate-y-0.5 transition-all duration-300"
              >
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="lg:col-span-2 space-y-6">
              <div className="flex items-center justify-between">
                ...
                <Link
                  href="/patient/appointments"
                  className="text-sm font-semibold text-[#16BCC8] hover:text-[#0ea5a9] transition-colors duration-200 flex items-center gap-1"
                >
              ...
              <div className="space-y-4">
                {upcomingApts.length > 0 ? (
                  upcomingApts.slice(0, 3).map((apt) => <AppointmentRow key={apt.id} apt={apt} />)
                ) : (
                  <div className="p-8 text-center bg-white border border-slate-100 rounded-2xl text-slate-400 text-sm">No upcoming appointments.</div>
                )}
              </div>
                ...
                <div className="bg-white border border-slate-100 rounded-2xl overflow-hidden shadow-card">
                  {recentPrescriptions.length > 0 ? (
                    recentPrescriptions.map((script: any, idx) => (
                      <div
                        key={idx}
                        className={`flex items-center justify-between p-4 ${idx !== recentPrescriptions.length - 1 ? 'border-b border-slate-50' : ''} hover:bg-slate-50/50 transition-all duration-200 group`}
                      >
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            ...
                            <p className="text-xs text-slate-400">
                              {new Date(script.appointmentDate).toLocaleDateString([], { month: 'short', day: 'numeric', year: 'numeric' })}
                            </p>
                    ...
                    <div className="p-8 text-center text-slate-400 text-sm">No prescriptions found.</div>
                  )}
                </div>
              </div>
            </div>
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                />
                <QuickActionCard icon={FileText} title="Records" desc="History" color="bg-gradient-to-br from-blue-500 to-blue-600" href="/patient/history" />
              </div>
