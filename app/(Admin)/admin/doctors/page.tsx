                ...
                <div className="bg-slate-100 p-1 rounded-lg flex self-start flex-wrap gap-1">
                  {(['All', 'Active', 'Pending', 'Suspended', 'Blocked'] as const).map((tab) => (
                    <button
                      ...
                      className={`px-4 py-2 text-sm font-medium rounded-md transition-all ${activeTab === tab ? 'bg-white text-teal-700 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
                    >
                      {tab}
                    </button>
                  ))}
                          ...
                          <tr key={doc.id} className="hover:bg-slate-50/50 transition-colors group">
                            ...
                            <td className="px-6 py-4">
                              <div className="flex flex-col text-sm text-slate-600">
                                <span className="flex items-center gap-1">
                                  <Mail size={12} className="text-slate-400" /> {doc.email}
                                </span>
                                <span className="flex items-center gap-1">
                            ...
                            <td className="px-6 py-4 text-right">
                              <div className="flex items-center justify-end gap-2">
                                <button onClick={() => setSelectedDoctor(doc)} className="p-2 text-slate-400 hover:text-teal-600 hover:bg-teal-50 rounded-lg transition-colors" title="View Profile">
                                  <Eye size={18} />
                                </button>
                                {doc.status !== 'Active' && (
                                  <button
                                  ...
                                  >
                                    {actionLoading === doc.id + '-verify' ? <Loader2 size={16} className="animate-spin" /> : <CheckCircle size={16} />}
                                  </button>
                                )}
                                <button
                                ...
                                >
                                  {actionLoading === doc.id + '-suspend' ? <Loader2 size={16} className="animate-spin" /> : <ShieldOff size={16} />}
                                </button>
                                <button
                                  ...
                                  title="Delete Doctor"
                                >
                                  {actionLoading === doc.id + '-delete' ? <Loader2 size={16} className="animate-spin" /> : <Trash2 size={16} />}
                                </button>
                              </div>
                            </td>
                          </tr>
                    ...
                    filteredDoctors.map((doc) => (
                      <div key={doc.id} className="p-4 space-y-3">
                        ...
                        <div className="flex gap-2">
                          <button onClick={() => setSelectedDoctor(doc)} className="flex-1 py-2 bg-slate-50 text-slate-600 text-sm font-medium rounded-lg border border-slate-200">
                            View Details
                          </button>
                          {doc.status !== 'Active' && (
            ...
            <div className="bg-white w-full max-w-2xl rounded-2xl shadow-xl overflow-hidden flex flex-col max-h-[90vh]">
                      ...
                      selectedDoctor.documents.map((doc, idx) => (
                        <div key={idx} className="flex items-center justify-between bg-white p-3 rounded-lg border border-slate-200">
                          <div className="flex items-center gap-3">
                            ...
                            <a href={doc} target="_blank" rel="noopener noreferrer" className="text-sm font-semibold text-teal-600 hover:text-teal-800 truncate flex-1">
                              View License PDF Document
                            </a>
                          </div>
                          <a href={doc} download={`license-${idx + 1}.pdf`} className="text-teal-600 hover:text-teal-800 text-xs font-bold">
                            Download
                          </a>
                        </div>
                      ))
              ...
              <div className="p-4 border-t border-slate-100 bg-slate-50 flex justify-end gap-3 flex-wrap">
                ...
                <button
                  onClick={() => {
                    setSelectedDoctor(null);
                    setConfirmDelete(selectedDoctor);
                  }}
          ...
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden">
            <div className="px-6 py-5 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
              <h3 className="font-bold text-slate-800 text-lg flex items-center gap-2">
                <UserPlus size={18} className="text-teal-600" /> Add New Doctor
              </h3>
            ...
            <form onSubmit={handleAddDoctor} className="p-6 space-y-4">
              ...
              <div className="space-y-1">
                ...
                <div className="relative">
                  ...
                  />
                  <button type="button" onClick={() => setShowAddPwd((p) => !p)} className="absolute right-3.5 top-1/2 -translate-y-1/2 text-slate-400 hover:text-slate-600">
              ...
              {addError && <div className="p-3.5 bg-red-50 border border-red-200 text-red-700 rounded-xl text-xs">{addError}</div>}
              <div className="pt-2 flex gap-3">
                <button type="button" onClick={() => setIsAddOpen(false)} className="flex-1 py-3 text-slate-500 hover:bg-slate-100 rounded-xl font-semibold text-sm transition-all">
                  Cancel
                </button>
                <button
                  type="submit"
          ...
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm overflow-hidden">
            <div className="p-6 text-center">
              ...
              <div className="flex gap-3">
                <button onClick={() => setConfirmDelete(null)} className="flex-1 py-2.5 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-xl font-semibold text-sm transition-all">
                  Cancel
                </button>
                <button onClick={() => handleDelete(confirmDelete)} className="flex-1 py-2.5 bg-red-600 hover:bg-red-700 text-white rounded-xl font-bold text-sm shadow-sm transition-all">
                  Delete
                </button>
              </div>
            </div>
