                    ...
                    {filteredUsers.length === 0 ? (
                      ...
                      filteredUsers.map((user) => (
                        <tr key={user.id} className={`hover:bg-slate-50/50 transition-colors group ${selectedIds.includes(user.id) ? 'bg-teal-50/30' : ''}`}>
                          ...
                          <td className="px-6 py-4">
                            <span className={`text-xs font-medium px-2 py-1 rounded-full ${user.role === 'VIP Patient' ? 'bg-indigo-50 text-indigo-700' : 'text-slate-600'}`}>{user.role}</span>
                          </td>
                          ...
                          <td className="px-6 py-4 text-center">
                            <span className="text-sm font-bold text-slate-700 bg-slate-100 px-2 py-1 rounded-md">{user.totalAppointments}</span>
                          </td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-white w-full max-w-lg rounded-2xl shadow-xl overflow-hidden animate-in zoom-in-95 duration-200">
            ...
            <div className="relative bg-teal-600 h-24">
              <button onClick={() => setSelectedUser(null)} className="absolute top-4 right-4 p-1.5 bg-white/20 hover:bg-white/30 text-white rounded-full backdrop-blur-md transition-colors">
                <X size={18} />
              </button>
            </div>
            ...
            <div className="px-6 pb-6 -mt-12">
              ...
              <div className="mb-6">
                ...
                <p className="text-slate-400 text-xs mt-1">
                  {selectedUser.phone} • Joined {selectedUser.joinedDate}
                </p>
              </div>
